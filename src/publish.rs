...
pub fn publish_every<M, W, S>(duration: Duration, source: AggregateSource, target: S) where S: Sink<M, W> + 'static + Send + Sync, W: Writer<M> {
...
pub fn publish<M, W, S>(source: &AggregateSource, target: &S) where S: Sink<M, W>, W: Writer<M> {
    let scope = target.new_writer();
    source.for_each(|metric| {
        match metric.read_and_reset() {
            ...
            ScoresSnapshot::Event { hit } => {
                ...
                let temp_metric = target.new_metric(MetricKind::Count, name, 1.0);
            ...
            ScoresSnapshot::Value { hit, sum, max, min } => {
                if hit > 0 {
                    ...
                    let temp_metric = target.new_metric(MetricKind::Gauge, name, 1.0);

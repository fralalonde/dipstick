use core::{MetricType, Value, MetricWriter, MetricSink, MetricDispatch, EventMetric, ValueMetric,
           TimerMetric, DispatchScope};
use std::sync::Arc;
...
pub struct ScopeWriter<C: MetricSink> {
    writer: C::Writer, //    properties: Hashmap;
}
...
impl<C: MetricSink> DirectDispatch<C> {
    pub fn new(target: C) -> DirectDispatch<C> {
        ...
        DirectDispatch {
            target,
            dispatch_scope: Arc::new(DirectScope {
                default_scope,
